...
public class CustomerRestaurant extends JFrame {
	...
	public void displayRooms() {
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/hms", "root", "H@mayun123");
			...
			// Fetch each row from the database and add to the table model
			while (rs.next()) {
	...
	public void addToDatabase() {
		...
		try {
			// Assuming GetConnection is a custom class that sets up the connection
			GetConnection connect = new GetConnection();
			Connection conn = connect.getConnection();

			// The following line should be removed if GetConnection already sets the correct database, username, and password
			// conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/hms", "root", "H@mayun123");

			String sql = "INSERT INTO roomcustomer(name, address, phone, roomtype, bedtype, price, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
			ps = conn.prepareStatement(sql);

			// Assuming name, addr, phone, roomtype, bedtype, tot are JTextFields or similar
